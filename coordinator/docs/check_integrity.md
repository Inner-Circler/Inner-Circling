...
check_integrity.py — the corruption gate: one sweep over every file this system keeps state in, run before a circle is allowed to open.

## SYNOPSIS

    python coordinator/check_integrity.py

(As a library: `sweep(root)` returns findings and a file count; `report_lines(findings)` formats them. `coordinator/circle.py` calls `sweep()` directly at open, and `coordinator/circle_audit.py` runs it as phase 0.)
...
**Two exit codes, from two callers.** Run bare, this script exits 1 on any finding and 0 when clean. When `circle.py` calls `sweep()` at open and finds something, *it* exits 2 and the circle is never opened — no transcript, no working-set entry, no API call.

## MAIN

    Run sweep() over the project root.
