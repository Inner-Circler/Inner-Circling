...
`--validate`
    Accepted but not read anywhere in `main()`'s dispatch logic; running with no phase-selecting flag already runs phases 0, 1, 2, 6, which the help text describes as the default. (See BUGS.)
...
- `--validate` is defined as a documented argparse flag ("run phases 0, 1, 2, 6 (the default)") but `main()` never reads `args.validate` anywhere in its dispatch chain. Running with no flags at all already falls through to exactly that phase sequence, so `--validate` is currently a no-op that changes nothing whether given or not — harmless, but the flag's own help text implies it selects a mode, when in fact every unhandled combination of flags reaches that same code path regardless.

- In `main()`'s final failure-summary line (`f"{len(unprocessed)} unprocessed circle(s); ..."`), the `unprocessed` variable referenced here is only ever assigned inside the `else` branch of the `--snapshot` check, deep inside the `try` block. If the `--snapshot` branch was taken, it returns early (before reaching this line) and the issue doesn't arise — but on any other path that raises an exception between entering the `try` block and the `unprocessed = phase1(run)` assignment (for instance, an exception during phase 0's own follow-on work, or, since this is inside the `try`, even the `--journal-*`/`--git-setup`/`--selfcheck` early-return branches — though those all return before this line is reached too), this line would raise `NameError: unprocessed` rather than reporting the failure that actually occurred. In practice every early-return branch does return before this line, so this looks unreachable as the code stands today, but it's a fragile arrangement: a future edit that adds a new early return without also returning before this line, or an exception from anywhere in the `try` body between its start and the `phase1()` call, would surface as a confusing `NameError` masking the real failure.

- `check_git()`'s dirty-tree message reports paths "outside the audit's output set" using `AUDIT_OUTPUT_RE`, but that pattern's own defining comment says it captures "exactly what an audit run is allowed to have left uncommitted" — yet `phase0()` calls `check_git()` before phase 7 has run (before any commit this invocation might make), so on a run that goes on to `--commit`, the dirty-tree check at phase 0 necessarily runs against the tree's state from a *previous* run's leftovers, not this run's own output, and the distinction between "expected because I'm about to write this" and "expected because a prior run already wrote this and nobody committed it yet" is not made anywhere in the message.
