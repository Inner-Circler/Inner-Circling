...
circle_close.py — the close verifier: proof that every part that spoke left a record, and that the record is still there afterwards.

## SYNOPSIS

    python coordinator/circle_close.py --reconcile --open-time YYYY-MM-DD_HHMM
    python coordinator/circle_close.py --short-term-only --open-time YYYY-MM-DD_HHMM
                                       [--write-report]
    python coordinator/circle_close.py
    python coordinator/circle_close.py --prune
    python coordinator/circle_close.py --claude-home /path/to/.claude
...
## EXTERNAL FILES
    ...
    parts/<name>/short_term_<OT>.md   READ — the note being verified
...
### short_term_status(path)
    if (the file does not exist) then { return (false, "missing"). }
    if (it cannot be read) then { return (false, the error). }
    if (it is blank) then { return (false, "empty"). }
    if (any of the four required sections is absent) then {
