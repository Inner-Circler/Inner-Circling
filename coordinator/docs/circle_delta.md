...
## COLLECT
    ...
    find the circle anchor {
        ask git for the tree's own stamped tag first (gitrepo.tag_name knows the lab stamp, R245),
    ...
    parse the transcript for the topic, Self's statement count (cmd-echoed graph rulings and
        Scribe notes excluded) and per-part counts {
...
READ: `work/logs/close_<OT>.json` (the gate and the per-part sizes), `circles/circle_<OT>.md`, `circles/commands_<OT>.toml`, `work/logs/spend_<OT>.json`, `work/logs/dream_<OT>.json`, `parts/*/remember.toml`, `parts/*/mid_term.md`, `self/proposals.toml` (via proposals.py), the four synthesis registers under `self/`, and git objects via read-only subprocess calls (`rev-list`, `log`, `show`, `cat-file`, `rev-parse`), each of which degrades to "no evidence" on any failure.
...
settings (the `circle_stats` read), roster, identity, transcript_store (whose markers import needs `memory/` on sys.path — this module adds it, the same hop circle.py makes), remember, proposals, markers, recall_index, gitrepo (tag_name only), atomic_write. Every project import is function-local and guarded; tomllib (tomli on 3.10) reads the registers raw.

## HUMAN I/O

Prints the report (or JSON) to stdout; every rendered line stays within 116 characters. `at_close(ot, emit)` is the `/close` hook: it emits through the caller's own channel, honours `enabled()`, and FAILS OPEN — a reporting fault must never fail a close.
