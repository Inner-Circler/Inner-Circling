...
circle_history.py — the CIRCLE_HISTORY register: SYNTHESIS's one durable record per circle, saying what the circle was and what moved.

## SYNOPSIS

    python coordinator/circle_history.py            the listing
    python coordinator/circle_history.py --init     write the empty register
    python coordinator/circle_history.py --show     the newest entry, in full

(As a library: `entries()` for every record, `most_recent()` for the newest, and `render_new(doc, circle, text)` — a pure function the phase-2 driver uses to build one entry without writing anything.)
...
**An oversize entry is REFUSED, never truncated.** A history entry cut off mid-sentence is a worse record than the previous entry standing alone, so `render_new()` raises rather than trims, the caller reports it, and the prior entry stays current.
...
`self_schema` for loading, saving and timestamping the register. `pathlib`, `sys` and `copy` from the standard library. The parent directory and `memory/` are put on the import path at import time, matching every other coordinator module's bootstrap.

## EXTERNAL FILES

    self/circle_history.toml    READ by every verb; WRITTEN only by --init here.
                                The real per-circle writes come from the phase-2
                                driver calling render_new() and saving the result.
