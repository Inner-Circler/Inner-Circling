...
dream_history.py — DREAM_HISTORY: the bounded standing summary of what Self's dreaming corpus has concluded.

## SYNOPSIS

    python coordinator/dream_history.py               state, writes nothing
    python coordinator/dream_history.py --bootstrap   the one bootstrap derivation
    python coordinator/dream_history.py --fold        fold every unfolded dream
    python coordinator/dream_history.py --show        the current DREAM_HISTORY, whole
    python coordinator/dream_history.py --selftest    the pure helpers, no model call
...
### truncate_words(text, cap)
    Split on whitespace.
    if (the word count is within the cap) then { return the text unchanged, and
        false. }
    otherwise return the first `cap` words rejoined, and true.
