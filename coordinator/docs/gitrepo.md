...
## NAME
gitrepo.py — the project's git surface: repository setup, identity resolution, ignore/attributes/hook maintenance, and path-scoped commits, shared by circle.py and nightly.py

## SYNOPSIS
    python coordinator/gitrepo.py
    python coordinator/gitrepo.py --identity

(All other functionality — `ensure_repo`, `ensure_config`, `ensure_attributes`, `ensure_hooks`, `ensure_ignore`, `untrack_ignored`, `commit_paths`, etc. — is a library API called from `circle.py` and `nightly.py`, not exposed as its own CLI verbs beyond `--identity`.)
...
4. **`git add -A` during an automated run.** A machine commit stages only the exact paths that run produced, via `commit_paths()`, so a nightly run at 01:11 cannot sweep up whatever a human happened to be editing at midnight.
...
This file has no `main()` function in the conventional sense used elsewhere in the project; its `if __name__ == "__main__":` block calls a function literally named `main()`, but that function only implements the `--identity` read-only CLI. All other capability is a library surface with no dispatcher.
    ...
    if (the "--identity" flag was not given) then {
        ...
        {
            resolve (user.name, user.email) via resolve_identity(), which prefers, in order: explicit flag (not offered by this narrow CLI), $IFS_GIT_NAME/$IFS_GIT_EMAIL, git's own configured user.name/user.email (local then global), or unset.
        }
...
## EXTERNAL FILES
Read:
- `ROOT/.env`, best-effort, via `_load_env()`, whenever `ensure_config()` or `main()`'s identity resolution runs.
- `ROOT/.gitattributes`, in `ensure_attributes()`, to check whether the `* -text` rule is already present.
- `ROOT/.gitignore`, in `ensure_ignore()`, to check which required ignore patterns are missing.
- `ROOT/.git/hooks/pre-commit`, in `ensure_hooks()`, to check whether an inner-circling hook (and which version) is already installed.
- The git index/working tree generally, via the various `git` subcommands (`status`, `config`, `remote`, `rev-parse`, `ls-files`).

Written:
- `ROOT/.gitattributes` — appended with `* -text` by `ensure_attributes()` if not already present.
- `ROOT/.gitignore` — appended with any missing entries from `REQUIRED_IGNORES` by `ensure_ignore()`; existing entries are never removed or reordered.
- `ROOT/.git/hooks/pre-commit` — written by `ensure_hooks()` if absent, or replaced if it is an out-of-date version of this project's own hook (identified by a shared `HOOK_FAMILY` marker comment); a hook belonging to someone else (no recognized marker) is left untouched with a warning.
- The git index — via `git add`, `git rm --cached`, `git commit`, `git tag`, `git config`, `git init`, called from `ensure_repo()`, `ensure_config()`, `untrack_ignored()`, and `commit_paths()`.

## NETWORK ACCESS
None. All operations are local `git` subprocess calls; no remote is ever added or fetched from by this module, and it refuses committing operations (`commit_paths()`, via `refuse_remote()`) if any configured remote could take the material off this machine. A remote on a local fixed or removable volume is permitted and is reported with the reason it passed.

## HUMAN I/O
Stdout, via the `log(status, message)` callback that every `ensure_*` / `commit_paths` function accepts from its caller (this module never constructs its own logger) — statuses observed being passed include `"ok"`, `"did"`, `"warn"`, `"fail"`, `"note"`. `main()` itself prints directly. No stdin. Exit codes from `main()`: 0 or 1 as described above; other functions return booleans or lists rather than process exit codes, since they are library calls.
...
### `available()`, `is_repo()`, `toplevel()`, `remotes()`, `dirty()`, `head()`
    {
        Thin wrappers over `run()` answering, respectively: is git installed and runnable; is ROOT (or an ancestor) a git repo; the git top-level directory if any; the configured remote names; the porcelain-status dirty lines; the short HEAD sha or "(no commits)".
    }
...
### `ensure_repo(log)`
    if (already a repo) then {
        ...
        run `git init`, log it, then return assert_isolated(log).
    }
...
### `resolve_identity(name=None, email=None)`
    {
        For (name, email) independently, in order of precedence: an explicit function argument (from a CLI flag, not offered by this file's own narrow CLI but available to callers like nightly.py); then $IFS_GIT_NAME/$IFS_GIT_EMAIL (shell or .env); then git's own local-then-global user.name/user.email; then unset (None). No hardcoded fallback identity exists deliberately — a hardcoded default was previously discovered sitting as an argparse default in nightly.py, and the module's docstring treats a wrong-but-present default as worse than an explicit unset, since it would commit history under someone else's name.
    }

### `ensure_config(log, name, email)`
    {
        Resolve identity via resolve_identity(). For each of user.name/user.email whose source is "unset", log a warning that git will refuse to commit until it's configured.
    }
...
The pre-commit hook template (`PRE_COMMIT`, currently v9) itself, once installed, runs `check_line_endings.py` unconditionally, then conditionally runs `issue_gate.py`/`test_issue_gate.py` (if `issues/` touched), a battery of self-check and best-practice/budget verifiers (if `parts/` or `self/` touched), `check_rulings.py` again (if `RULINGS.md` touched), `prompt_capture.py --verify` (if `prompts/` touched), and `check_lint.py` (if `coordinator/` or `scripts/` touched) — this is documentation of the hook's *content*, not of gitrepo.py's own runtime behavior, since the hook runs later, in a separate `sh` process, at commit time.
...
### `commit_paths(paths, message, log, tag=None)`
    if (ROOT is not a git repository) then {
        log a warning and return False.
    } else if (assert_isolated fails, or refuse_remote returns True) then {
