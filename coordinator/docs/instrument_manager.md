...
python coordinator/instrument_manager.py --show    print the BLOCK 1 text
```
...
mentions a part — the B29/R123 class, and the choice
`parts_prompt_projection.part_identity_tail_render()` made.

**The Soul carries the demographics too**, through the separate
`[context]` mechanism in `parts/soul/part.toml` — `part_identity_tail_render()` renders
...
this file**. `instrument_read()` returns `{}`, `instrument_block_render()` returns `""`,
`instrument_verify({})` returns
...
`record_verify.py`'s business, not this module's.
...
instrument produced. Same shape as `practice_verify`'s verbatim-arrival
...
`coordinator/tests/test_instrument_manager.py` asserts each refusal against a mutated
