...
## DEPENDENCIES
Standard library: `pathlib`, `re`, `sys`, `textwrap`, `__future__.annotations`. TOML: `tomllib` (Python 3.11+) with a fallback import of `tomli` (identical parser) for older interpreters; `tomli_w` for writing. Two coordinator modules: `atomic_write` (`record_atomic_write`, the one way a node is written) and `record_paths` (the bound group's issues/ directory). No other `issue_*.py` is imported — this module sits at the base of the issue-graph dependency chain, and the other `issue_*.py` files import it, not the reverse.
