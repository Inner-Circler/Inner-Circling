...
proposals.py — the PROPOSE register: what a part has asked Self to rule on, and what Self ruled.

## SYNOPSIS

    python coordinator/proposals.py

(As a library: `entries()`, `by_id(id)`, `pending()` for the vetting queue, `stage(kind, text, sources, circle)` when a circle closes, and `approve(id)` / `deny(id)` / `supersede(id, circle)` when Self rules.)
...
**Nothing here touches the issue graph.** The graph-writing half of approving a command row lives in the circle driver; this module stays a pure register that knows only its own state. `approve()` and `deny()` are called only after that orchestration has already succeeded — or, for a text row, immediately, because there is nothing else to satisfy.
...
## EXTERNAL FILES
    ...
    self/proposals.toml    READ by every verb; WRITTEN by stage(), approve(),
                           deny() and supersede().
...
### stage(kind, text, sources, circle)
