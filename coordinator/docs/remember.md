...
remember.py — the REMEMBER register: a part's own note to its future self, private to that part.

## SYNOPSIS

    python coordinator/remember.py --part <part>              every record on file
    python coordinator/remember.py --part <part> --project    what the part is shown

(As a library: `entries(part)`, `add(part, guard, text, ...)`, `has_remembered(part, guard)`, `project(part)`, `block_settled(part, cutoff)` for BLOCK 3 and `block_tail(part, cutoff)` for BLOCK 4.)
...
## MAIN
    ...
    if (--project was given) then {
        print the part's projection as the part would see it, or "(nothing on
        file)", then the projection's own statistics; return 0.
    }
    otherwise {
...
## COMMAND-LINE ARGUMENTS
    ...
    --project       show the rendered projection — what the part is actually
                    shown — rather than the raw records. Default: off.

## DEPENDENCIES

`self_schema` for load, save and timestamps; `write_guard` for the write path; `mid_term` for the refresh cutoff that splits settled from tail. `pathlib`, `sys`, `datetime`.

## EXTERNAL FILES

    parts/<name>/remember.toml    READ by every verb; WRITTEN by add().
...
### block_settled(part, cutoff) / block_tail(part, cutoff)
The two projections, for BLOCK 3 and BLOCK 4 respectively, each with its own header text.

## BUGS

None found. One thing that reads as an omission and is not: `add()` does not enforce the one-per-circle allowance. That check belongs to the caller, and centralising it here would have put it below the two different call sites that need to answer it differently.
