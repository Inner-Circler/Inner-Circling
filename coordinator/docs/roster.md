...
## NAME
roster.py — the single canonical list of parts (directory / display tag pairs, canonical and alphabetical orderings, and lookup dicts in both directions), replacing what the docstring describes as thirteen-plus hardcoded copies scattered across the codebase.

## SYNOPSIS
    python coordinator/roster.py
...
## DESCRIPTION
The docstring frames this file as consolidation work: before it existed, the ordered list of parts was typed out by hand in roughly thirteen places across eight files (with `coordinator/circle_close.py` alone contributing two more copies, `TAG_TO_DIR` and `MAX_MEMBERS`), and each copy was a place a part could go silently missing — adding a part meant finding and editing every one of ten copies, and a missed one would not raise an error, it would simply leave that reader blind to the new part. `roster.py` is now the one place that changes.

That consolidation has since gone further than the docstring's original scope: `ROSTER`, `ALT_TAGS`, `IDENTITY_TAILS`, and `PROBLEMS` are no longer a hand-typed table at all. `scan()` reads `parts/` directly — every immediate subdirectory containing a `part.toml` is a part, its `tag` field supplies the display Tag, its optional `alt_tags` list supplies any historical spellings, and its optional `identity_tail` string supplies that part's own speaking rules, appended in BLOCK 3 every circle (R303; arrived R246 as `--minimal`'s block-4 suffix, moved out of `prompt_build.py` where it had been keyed by a part name — the mode itself retired R360). A directory with no `part.toml`, or one that fails validation (missing/invalid tag, a tag containing transcript-grammar characters, a tag colliding with another part's Tag/alt_tag or with Self's own name), is excluded from the roster and reported in `PROBLEMS` rather than silently skipped.

The docstring also explains why ordering is alphabetical rather than a separately maintained canonical order: most readers in the codebase once used one canonical circle ordering, while three others (`ifs_model.PARTS`, `recency.PARTS`, and `check_best_practices.ADDRESSEES` after its "All parts" entry) used an alphabetical-by-directory ordering instead. A canonical order cannot survive a scan — there is nothing in a directory to derive it from — so alphabetical-by-directory-name is now the only order there is; `ALPHA_DIR_NAMES` is kept as an alias of `DIR_NAMES`, not a second distinct ordering, so the three readers that historically expected that name do not need to change.

A separate note documents a historical rename: the seventh part's display tag was changed from its earlier two-word form to its current shorter form ("Part#" in both cases, per this project's redaction convention) on 2026-08-07. Fifty-six existing lines across the corpus still carry the earlier two-word form and constitute the historical record, so any reader that walks past older transcripts (`circle_close.py`, `issue_gate.py`) must still be able to resolve it; a reader that only ever sees newly generated output does not need the older form.
...
### `ROSTER`, `ALT_TAGS`, and the derived lookup structures
    {
        ...
        scan(): every immediate subdirectory of parts/ (skipping
...
### `statement_re(tags)`
    {
        ...
        circle_close.py and nightly.py each hand-rolled a copy and the
...
### `verify()`
    {
        re-run scan() against base (default parts/), fresh — not the
    ...
    start from every problem scan() itself already reports (no
    ...
    Self's own display name (case-insensitively) — a check scan() itself
    cannot make, since scan() does not know who Self is;
    ...
    since 2026-08-23 scan() also reports every [context] shape problem
    context_problems() finds (non-string purpose, a question without a
...
### `context_problems(dir_name, doc)`, `read_context(dir_name, base=None)`, `write_context(dir_name, answers, base=None)`
    ...
    write_context: refuse a key answering no question or a non-string value;
        ...
        the new text must load and verify BEFORE it lands; atomic_write()
