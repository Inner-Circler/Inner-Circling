...
self_observation_manager.py — the SELF_OBSERVATION register: what synthesis noticed about the CIRCLE as a working body, written to Self.

## SYNOPSIS

    python coordinator/self_observation_manager.py            the listing
    python coordinator/self_observation_manager.py --init     write the empty register
    python coordinator/self_observation_manager.py --show     the newest entry, in full
...
(As a library: `self_observation_read()`, `self_observation_latest_read()`, the pure `self_observation_new_render(doc, circle, text, note, salience, continues)` used by the phase-2 driver, the pure `self_observation_manual_render()`/`self_observation_continue_render()`/`self_observation_retire()`/`self_observation_purge()`, `self_observation_list()`/`self_observation_show()`, and the impure load-mutate-save wrappers `self_observation_manual_add()`/`self_observation_continue_add()`/`self_observation_retire_now()`/`self_observation_purge_now()` the five room verbs call.)
...
```
/observation-add <text>              self_observation_manual_render() + save — Self's own
                                     entry, no circle required
/observation-list [<n>] [--all]      self_observation_list()/self_observation_show() — retired hidden
                                     by default, --all shows everything
/observation-continue <id> <text>    self_observation_continue_render() + save — a NEW
                                     record chained onto <id>; <id>'s own
                                     record is never touched
/observation-retire <id>             self_observation_retire() + save — soft delete: sets
                                     retired=true, stays in the file
/observation-purge <id> <id>         self_observation_purge() + save — replaces TEXT with a
...
## EXTERNAL FILES
    ...
    self/self_observation_log.toml   READ by every verb; WRITTEN here only by
                                     ...
                                     the phase-2 driver via self_observation_new_render(). The
...
### self_observation_read() / self_observation_latest_read()
...
### self_observation_new_render(doc, circle, text, note=None, salience=None, continues=False)
