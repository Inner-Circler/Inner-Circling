...
WHY IT IS ITS OWN FILE. gitrepo.py was 3,470 lines and 2,676 of them were this: the
pre-commit script, the two backup-push scripts, and the installer that renders and checks
them. That is not a second job hiding in a file — it is the LARGER job, with a different
audience. The git SURFACE that remains in gitrepo.py is called by nine modules; everything
here is called by circle_audit.py's --git-setup and by nothing else. The 2026-09-09 cohesion
map measured that split and it was the only module in the tree where the two halves had
genuinely different callers.

AND MOST OF IT IS NOT PYTHON. PRE_COMMIT is 1,976 lines of /bin/sh inside a Python string,
...
THE NAME. GIT became a class word in the grammar on 2026-09-09 (the operator: "GIT is an
option and needs to be distinct from SYSTEM, add the word"), and HOOK with it, so <CLASS>_<what>
resolves here rather than borrowing SYSTEM the way every function in it still does.
...
#
# THE REAL REASON IS THE ONE THIS ENTRY GAVE ALL ALONG: its checks WERE stale. Run with
# --fast it was 20 pass and 15 FAIL, against the "16 of its checks are stale" this entry had
# said since it was written. What had expired was only its PRECONDITION ("wired once B51(3)
# and R202 are realigned").
#
# SETTLED 2026-09-09. R491 ("2 - diagnose and wire") landed at v171: all fifteen were judged
# one at a time, every one was STALE and none a regression, and each is now dated to the
# ruling that moved it. The suite runs in the battery at :2322 and PASSES — exit 0, 62
# checks, 0 failures, 64 seconds. This paragraph still said "the realignment is still owed"
# until today (audit-register 2026-09-09 #65), a day after the realignment landed.
#
...
HOOK_MARK = "# inner-circling pre-commit v179"
...
# corruption gate (memory/record_verify.py) and its probe shipped that
...
if [ -n "$IC_RECORD" ]; then
    ...
    # v178, 2026-09-09. THE EXEMPLAR COUNTER COULD NOT SEE THE FILE THE EXEMPLARS ARE IN.
    # This trigger named coordinator/process_core.md and not coordinator/process_ifs.md, while
    # test_annotation_exemplars.py below reads the COMPOSED rulebook — its own core_text()
    # docstring says so in as many words: "The [proposed:] exemplars live in the IFS layer, so
    # reading process_core.md alone would miss them." So the one file that holds the counted
    # exemplars did not fire the suite that counts them.
    # CAUGHT BY BEING WRONG: audit-register 2026-09-09 #22 added an /issue-label-update
    # exemplar to the IFS layer, that commit passed green because this suite never ran, and it
    # refused a LATER commit whose own trigger reached it. v45's shape exactly — "a case that
    # matches no path never runs" — one layer down, where the path exists and the case did not
    # name it. process_band.md is deliberately NOT added: circle_identity_text_render() composes
    # the DEFAULT layer, so this suite reads the IFS one and never the band's.
...
#
# v179, 2026-09-09. THE OPEN STEP LEFT circle.py, SO FIVE ARMS NAMING IT GAINED
# coordinator/circle_open.py. Every one of the five was written to be triggered by "the
# circle driver", and after the carve the driver is two files: main() parses the flags and
# runs the Self> loop, circle_open() does everything between. An arm still naming only
# circle.py would go on matching — and would silently stop covering its subject on any
# commit that touched the open step alone. v45's lesson, from the side where the case
# still fires: a trigger that names a path is not the same as a trigger that names a
# SUBJECT, and a carve is exactly when the two come apart.
...
|*.gitattributes*|*.gitignore*)
    ...
    # v177, 2026-09-09 (audit-register 2026-09-09 #12). THE TRIGGER WAS NARROWER THAN THE
    # SURFACE IT DEFENDS. Measured against sanitize._collect_targets() — the ship set itself,
    # 165 files — the four patterns above the added line missed FORTY-THREE of them:
    #
    #   groups/  20   including groups/ifs/self/ -- self.md, remember.toml, topics.toml and
    #                 the rest, which are the operator's OWN registers and so the
    #                 highest-value PII surface in the tree
    #   work/     7   the shipped README.md files under work/*/
    #   ui/       6   ui/circling.py among them — the shipped entry point a person RUNS
    #   docs/     4
    #   root      6   README.md, LICENSE, NOTICE, requirements.txt, .gitattributes, .gitignore
    #
    # The audit that found this said 11 files; the measurement says 43, and the number matters
    # because ui/ and groups/ are both in it. Cost of widening: sanitize --dry-run is 0.9s
    # against the whole ship set, measured, so this is not a trade at all.
    #
    # THE DURABLE FIX IS NOT A LONGER PATTERN. This list is a hand-maintained restatement of
    # sanitize._collect_targets(), which is exactly the drift shape sanitize.py's own man page
    # warns about ("a third module restating it would be that defect a third time"). A shell
    # case cannot import Python; the honest version is a probe asserting that every ship-set
    # path matches this trigger, and that is not built.
    quiet packaging/sanitize.py --dry-run
    # v177, 2026-09-09 (audit-register 2026-09-09 #8). THE SIBLING SCAN JOINS ITS
    # SIBLING. sanitize.py asks "does this leak something private"; this asks "does
    # this point at something absent", and shipped_references.py's own man page says
    # "the two together cover the shipped surface". Only one of the two was wired to
    # anything: the other exited 1 on 165 HIGH findings that nothing consulted, not
    # the hook, not publish.py, not docs/publishing.md.
    #
    # It gates rather than advises because its HIGH tier is now honest. It refused on
    # comments and docstrings until the same day's fix (#10), and 164 of those 165
    # are gone — 113 were misgraded surface, and the rest were real citations and
    # paths that this commit series took out of shipped help text. The tier is 0
    # today, so the gate starts from a clean tree rather than from a suppression.
...
|*coordinator/circle.py*|*coordinator/command_surface.py*|*coordinator/stream_redaction.py*)
    ...
    # v177, 2026-09-09: THREE coordinator MODULES JOINED THIS TRIGGER, closing five findings of
    # audit-register 2026-09-09 (#3, #4, #6, #7, #26) that are all one defect wearing five faces.
    # Each is a module in coordinator/ whose only executing cover lives in ui/tests/, and whose own
    # trigger did not reach it:
    #
    #   coordinator/circle.py            --resume's whole branch (circle_transcript_resume_read has
    #                                    NO caller in any suite; ui/tests/test_ticker_bridge.py:788-830
    #                                    is the only thing that drives it), the same-minute overwrite
    #                                    refusal (test_ticker_bridge.py:866), and the /issue ruling
    #                                    path at :1891-1922, which ui/tests/circle_test.py submits for
    #                                    real. circle.py DID trigger test_circle_engine.py, but that
    #                                    suite forwards only a read verb and a [proposed:] form, so it
    #                                    enters none of the three.
    #   coordinator/command_surface.py   COMMANDS/PANE_OF — the pane-refusal contract both panes
    #                                    dispatch on, proved only by ui/tests/test_circling.py.
    #   coordinator/stream_redaction.py  what the circle pane redacts with.
    #
    # test_hook_template.py property 5 cannot see this class: it asks whether a SUITE's own subject
    # triggers it, never whether a MODULE's trigger reaches the suites that cover it. The durable
    # version is that missing property; this is the trigger it would have demanded.
...
|*memory/record_verify.py*|*memory/issue_gate.py*\
...
case "$FILES" in *packaging/sanitize.py*|*packaging/package.py*|*packaging/ignore.txt*)
    ...
    # v177, 2026-09-09. test_hook_template.py now asserts that every path
    # sanitize._collect_targets() returns matches the shipped-surface case above. That
    # assertion goes stale from EITHER end: the hook's patterns, or the ship set they
    # restate. The hook's end is covered by the case two blocks up; this is the other end,
    # and it runs the one suite rather than that block's four.
    run coordinator/tests/test_hook_template.py
esac
...
# coordinator/git_hook_script.py — _backup_push_hook(). Editing this file changes
...
def _ensure_one_backup_hook(d: pathlib.Path, name: str, body: str, mark: str,
    ...
    try:
        ...
        pass
    # SAY WHAT IS TRUE FOR WHOEVER IS READING IT. This said "the backup disk now
    # learns about this event", which is true HERE and meaningless in a bundle: a
    # recipient has no `backup` remote, these hooks no-op without one, and the
    # sentence named a thing they do not have. Found 2026-09-10 by
    # shipped_references.py, on the commit that first put this module in the ship
    # set — the gate's whole subject is a shipped line a stranger cannot resolve.
