#!/usr/bin/env python3
"""
instruments.py — the one reader of `self/instruments.toml`, and the gate over
what it puts into a prompt.

    python coordinator/instruments.py           verify the register
    python coordinator/instruments.py --show    print the block, and whose it is
...
`instruments.toml` at all, and `block()` returns ε there exactly as it does
...
CORRUPT file is `check_integrity.py`'s business, not this module's.
...
def main() -> int:
    if "--show" in sys.argv:
        who = ((load().get('block') or {}).get('part') or '').strip()
        text = block(who) if who else ''
