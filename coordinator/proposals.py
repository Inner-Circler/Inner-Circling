...
markers.py's `_propose_command_shape()` and carried here as `kind`; since
...
both, and the fixtures still exercise "text":
    ...
    "command"   `text` parses as a real command — identical syntax to a
                valid cmd> input (issue_commands.parse()'s own
...
RECORD SHAPE:
    ...
    circle       STAGING ONLY: the circle_<OT> transcript ref of the
                 ...
                 form by stage() itself (circle_ref(), R243 2026-08-19;
...
own caller. `approve()`/`deny()` here are ONLY ever called after that
...
def _doc() -> dict:
    return SS.load(PROPOSALS) if PROPOSALS.is_file() else {
...
def circle_ref(circle: str) -> str:
    ...
    open times gain circle_: staging is live-only (markers.
    stage_propose_proposals returns [] for sandbox), so a bare OT can
    only mean circle_. Prefixed and empty values pass through unchanged.

    RULED 2026-08-19 (R243): stage() applies this at the moment a row
    ...
    file. Rows staged before R243 carried the bare OT (markers passed
...
def stage(kind: str, text: str, sources: list[str], circle: str) -> str:
    """Append ONE new P-id row, state='proposed'. The caller (circle.py's
    coalesce_propose_proposals()) decides what a converged group's
    ...
    is kept IN THE ORDER GIVEN — coalesce_propose_proposals() passes
...
def supersede(pid: str, by_circle: str) -> tuple[bool, str]:
    """A pending row Self has ALREADY RULED IN THE ROOM by a direct command
    — 2026-08-21 (F6, *"yes"*). TOMBSTONED like deny(), never deleted: state
...
def approve(pid: str) -> tuple[bool, str]:
    ...
    issue_commands.precheck()/apply() successfully before calling this —
...
def deny(pid: str) -> tuple[bool, str]:
    ...
    2026-08-12 ruling every propose-class deny() follows
...
def listing() -> str:
    ...
    listing() shape — pending first (what needs a ruling), then
    settled."""
    rs = entries()
    if not rs:
        return "  no proposals"
...
def main() -> int:
    print(f"\n  {len(pending())} pending, {len(entries())} total\n{listing()}")
    return 0
