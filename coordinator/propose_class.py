...
relation_proposals.py into proposals.py, and this module now holds the
ONE copy of what remained duplicated between proposals.py and
check_best_practices.py — "parameterized by table name, id prefix, and
...
on resolution, and the save (post-mutate hook, then self_schema.save).
...
`BPX.BP` and `BPX._doc` as module attributes — a value captured here at
import time would go stale the moment a test rebinds one, the same
late-binding trap seam.py documents for emit/read_line. self_schema is
reached as a module attribute (`SS.save`) for the same reason — tests
swap it — and always with the exact (path, doc, table, order) argument
shape those harnesses' replacement lambdas accept.

THE TWO REGISTERS' `_now()` DIVERGED at build time — proposals.py
stamped UTC with offset, check_best_practices.py naive local — and
...
class ProposeClass:
    ...
    def save(self, doc: dict) -> None:
        ...
        SS.save(self.path_fn(), doc, self.table, self.order)
