#!/usr/bin/env python3
"""
quote_as_mark.py — QUOTE-AS-MARK. Self quotes a part's own words back to
it at the circle prompt; that quoting IS a ratification, and this module
is what records it.

    python coordinator/quote_as_mark.py --demo
...
have chosen to remember"* heading (remember.project()'s BLOCK 4 header)
...
it — `/recall` — and reaches the part, if at all, the way every other
...
approximately right at best (markers.py's own note on why markers beat
...
`[hold]` half of A10 is exactly the story of an opt-in marker the room
under-used. So it FAILS OPEN, like unruled_proposals(): a missed quote
...
WRONG mint is what the rules below are shaped to prevent.
    ...
    BY A PART          Self quoting Self is not a ratification. Scribe
                       notes and withheld entries are already outside
                       commands.statements(), which is the numbering
                       this cites.
...
commands.statements(), 1-based, imported rather than re-derived. That
...
# landed_numbers(). ONE format, spelled twice on purpose — a builder and
...
def quoted_spans(text: str) -> list[str]:
    ...
    for m in QUOTE_RE.finditer(text):
        ...
        if len(fold(span).split()) >= MIN_QUOTE_WORDS:
            out.append(span)
    return out
...
def lands_text(n: int, display: str, span: str) -> str:
    ...
    and the tail is for a human reading `/recall <n>` — one string, so
    the dedupe key cannot drift away from what is displayed."""
...
def _part_statements(transcript: list[dict]) -> list[tuple[int, dict]]:
    ...
    return [(n, e) for n, (_i, e) in enumerate(CM.statements(transcript), 1)
            if e["speaker"] != ID.SELF_ID]


def detect(transcript: list[dict], text: str) -> list[dict]:
    ...
    folded = [(n, e, fold(e["text"])) for n, e in prior]
    out, seen = [], set()
...
def landed_numbers(guard) -> set[int]:
...
def apply_quote_as_mark(guard, transcript: list[dict],
                        text: str) -> list[dict]:
    ...
    to press a point does not mint a second row. remember.has_remembered()
    skips class-bearing records for the other half of the same rule."""
    import remember as RM
    hits = detect(transcript, text)
    if not hits:
        return []
    done = landed_numbers(guard)
    written = []
    for h in hits:
        if h["n"] in done:
            continue
        rec = RM.add(RM.SELF, guard,
...
def main() -> int:
    ...
    nothing to verify here that test_quote_as_mark.py does not verify
    ...
    for h in detect(transcript, said):
