#!/usr/bin/env python3
"""
settings.py — self/settings.toml, the one place a person changes what the
code would otherwise decide. Approved 2026-08-28 (R379).

    python coordinator/settings.py            every setting, its value and its source
    python coordinator/settings.py --check    the gate: SPEC and the call sites agree
...
this project — remember.py's own comments say four of its numbers are
...
has gone wrong before, so nothing moves. The call site reads
    ...
    MAX_SINCE_SELF = SET.value("statements_per_part", 2)
...
discipline instruments.py already uses, and the reason a fresh clone, a
...
PARTS NEVER SEE ANY OF THIS, and three separate things have to hold:
    ...
    3  the verbs are EXCLUDED from the proposable set. THIS IS THE SHARP
       ...
       PROPOSABLE_COMMANDS — markers.py reads those two and has since R267
       ...
       proposable; coordinator/tests/test_settings.py asserts the
...
WHEN A CHANGE TAKES EFFECT. `applies` is "immediate" or "next_circle".
    ...
    next_circle   written to [pending], and folded into [active] by
                  fold_pending() at the checkpoint circle.py already runs
...
# rounds.py's retry string says 150, process.md says a stale 100). Deferred
...
SPEC: tuple[Setting, ...] = (
    ...
    _s("remember_word_cap",
       ...
       "coordinator/remember.py::AUTHORED_WORD_CAP", unit="words"),
    _s("remember_budget",
       "How much of a part's remembered notes reach it each circle",
       "NUMERIC_STRING", 200000, "dev", "next_circle",
       "coordinator/remember.py::BUDGET", unit="characters"),
    _s("salience_lift",
       "How far a charged memory may jump ahead of a newer one",
       "NUMERIC_STRING", 20, "dev", "next_circle",
       "coordinator/remember.py::SALIENCE_K", unit="places"),
    _s("remember_gate_ceiling",
       "The largest single remembered note the gate will admit",
       "NUMERIC_STRING", 200000, "dev", "next_circle",
       "coordinator/remember.py::GATE_CHAR_CEILING", unit="characters"),
    _s("mid_term_budget",
       "How long each part's distilled identity may be",
       "NUMERIC_STRING", 50000, "dev", "next_circle",
       "coordinator/mid_term.py::BUDGET", unit="characters"),
    _s("topics_budget",
       "How much of the open topics is carried into the briefing",
       "NUMERIC_STRING", 50000, "dev", "next_circle",
       "coordinator/topics.py::BUDGET", unit="characters"),
    _s("dream_staleness",
       "Circles before a dream entry counts as stale",
       "NUMERIC_STRING", 500, "dev", "immediate",
       "coordinator/recency.py::THRESHOLD", unit="circles"),
    ...
    _s("dream_max_tokens",
       ...
       "coordinator/inter_circle.py::DREAM_MAX_TOKENS", unit="tokens"),
    _s("synth_max_tokens",
       "The ceiling for the one circle-wide synthesis after a circle",
       "NUMERIC_STRING", 64000, "dev", "next_circle",
       "coordinator/inter_circle.py::SYNTH_MAX_TOKENS", unit="tokens"),
    _s("derive_max_tokens",
       "The ceiling for distilling one part's identity",
       "NUMERIC_STRING", 64000, "dev", "next_circle",
       "coordinator/mid_term.py::DERIVE_MAX_TOKENS", unit="tokens"),
    ...
    # already committed — and circle_delta.enabled() reads at use, not
    ...
    # should see the corrected figure at once. They are here because
    # llm_client's own comment says the four rise on 2026-09-01, and until
    # now the only way to record that was to edit Python.
    #
    ...
    # built now (model_rate_rows() above, `[[model]]` in the register), the
    ...
    # USE: llm_client.preflight_api() makes one real call before anything is
...
def value(key: str, default):
    ...
    if spec is None:                          # not a declared setting at all
        return default
    raw = (load().get(ACTIVE) or {}).get(key)
    if raw is None:
        return default
    ok, parsed, why = coerce(spec, raw)
    if not ok:
...
def coerce(spec: Setting, raw):
    ...
    value() above and by the editor before it writes — so a hand-edited file
...
def visible(dev: bool) -> tuple[Setting, ...]:
...
def tuning(provider) -> dict:
    ...
    table = (load().get(TUNING) or {}).get(_slug(provider), {}) or {}
    ...
    for key, raw in table.items():
        ...
        if knob is None:
            ...
            continue
        import roster as R
        why = R.check_value(knob.as_question(), str(raw))
        if why is not None:
            ...
            continue
        out[key] = R.canonical(knob.as_question(), str(raw))
    return out


def tuning_defaults(provider) -> dict:
    return {k.key: k.default for k in getattr(provider, "TUNING", ())}


def write_tuning(provider, key: str, raw) -> tuple:
    ...
    import roster as R
    mine[key] = R.canonical(knob.as_question(), str(raw))
...
def flush_corrections(emit=None) -> int:
...
def pending() -> dict:
    ...
    return dict(load().get(PENDING) or {})


def active() -> dict:
    return dict(load().get(ACTIVE) or {})


# ------------------------------------------------------------------- writing
def write(key: str, raw, *, now: bool) -> tuple[bool, str]:
    ...
    ATOMIC, and through self_schema's own dumper — a settings file this
...
def _dump(doc: dict) -> None:
    ...
    lines = ["# self/settings.toml — what THIS installation has changed.",
             "#",
             "# Written by coordinator/settings.py; edit it here or through",
             ...
             "# `python coordinator/settings.py`.",
             "#",
             "# [active]  in force now.",
...
# --------------------------------------------------------------------- gate
def _source_default(owner: str, key: str = "") -> tuple[str | None, str]:
    """The literal the owning module passes to value(), GREPPED FROM SOURCE
    ...
    side. `MAX_SINCE_SELF = SET.value("statements_per_part", 2)` documents 2,
    ...
    rhs = m.group(1).strip()
    inner = re.match(r'(?:SET\.)?value\(\s*["\']' + re.escape(key or name)
                     + r'["\']\s*,\s*(.+?)\s*\)$', rhs)
...
def show(key: str, v) -> str:
    ...
    BOOL does not: the register stores a real TOML `true`, `_source_default`
...
def _GATES() -> tuple:
    """The gate vocabulary, from roster — IMPORTED, NOT COPIED. It is the same
    list part.toml questions are held to, and two copies of it is the defect
    check_one_home.py exists for."""
    import roster as R
    return R.GATES


def length_rule_faults(core_src: str) -> list[str]:
    """What is wrong with process_core.md's length rule, or []. Takes the
    TEXT rather than reading the file, so a probe can hand it a broken one —
    a refusal nothing ever demonstrates is the defect this project treats as
    worse than a loud failure."""
    out = []
    for token in ("{LENGTH_AIM}", "{LENGTH_MAX}"):
        if token not in core_src:
            out.append(f"process_core.md no longer carries {token} — the "
                       f"length rule has been written back as a literal")
    for line in core_src.splitlines():
        if line.startswith("**Length:**") and any(c.isdigit() for c in line):
            out.append(f"process_core.md's length line carries a bare "
                       f"number: {line.strip()!r}")
    return out


def check() -> int:
    """SPEC and the code agree, and no settings verb is proposable.

    THREE THINGS, and the third is the one that matters. A settings verb a
    part could name in [proposed: ...] would hand the parts the surface this
    register exists to keep from them, and it would do it silently — a verb
    lands in DEV_CMD_HEADS as a matter of routine when it becomes
    dispatchable."""
    bad: list[str] = []
    seen: set[str] = set()
    for s in SPEC:
        if s.key in seen:
            bad.append(f"{s.key}: declared twice")
        seen.add(s.key)
        if s.data_type not in TYPES:
            bad.append(f"{s.key}: data_type {s.data_type!r} is not one of "
                       f"{TYPES}")
        if s.audience not in ("user", "dev"):
            bad.append(f"{s.key}: audience {s.audience!r}")
        if s.applies not in ("immediate", "next_circle"):
            bad.append(f"{s.key}: applies {s.applies!r}")
        # EVERY SETTING IS MECHANISM-FACING, so every setting declares how it
        # is kept valid. Refused rather than defaulted: an unknown gate here
        # would be a setting nobody had decided about, wearing the look of one
        # that had been.
        if s.gate not in _GATES():
            bad.append(f"{s.key}: gate {s.gate!r} is not one of "
                       f"{', '.join(_GATES())}")
        lit, why = _source_default(s.owner, s.key)
        if lit is None:
            bad.append(f"{s.key}: {why}")

    # The call site must READ the register, or the setting is declared and
    # dead — the v45 defect ("triggered and uninvoked") in this register's
    # own shape.
    import re
    for s in SPEC:
        path = s.owner.partition("::")[0]
        try:
            src = (ROOT / path).read_text(encoding="utf-8")
        except OSError:
            continue
        if not re.search(r'value\(\s*["\']' + re.escape(s.key) + r'["\']',
                         src):
            bad.append(f"{s.key}: {path} never calls settings.value() for it")

    # THE EXCLUSION. Asserted here as well as in the suite, so the gate that
    # runs on a settings change is the gate that checks it.
    try:
        import command_surface as CS
        for verb in SETTINGS_VERBS:
            for name in ("PROPOSE_SUBSET_COMMANDS", "PROPOSABLE_COMMANDS",
                         "DEV_CMD_HEADS"):
                if verb in getattr(CS, name, ()):
                    bad.append(f"{verb} is in {name} — a part could name it "
                               f"in a proposal; settings are Self's alone")
        # The list cannot go stale behind this check: a "/settings-" verb
        # added to the surface and forgotten in SETTINGS_VERBS would escape
        # every test above, which is v45's "triggered and uninvoked" shape.
        stray = [c for c in getattr(CS, "KNOWN_CMDS", ())
                 if c.startswith("/settings-") and c not in SETTINGS_VERBS]
        if stray:
            bad.append(f"SETTINGS_VERBS does not list {stray} — a settings "
                       f"verb nothing here is checking")
    except Exception as e:                                     # noqa: BLE001
        bad.append(f"command_surface did not import: {e}")

    # THE LENGTH RULE STAYS A PLACEHOLDER, 2026-08-28. The rule reaches the
    # parts as prose in process_core.md, and the whole point of making its
    # numbers settings is that the sentence carries no literal. Someone
    # editing that file for any other reason could type "150" back in, and
    # nothing else in the battery reads that line — the setting would go on
    # existing while the parts were told a number nobody could change.
    #
    # SOURCE TEXT, not load_shared(): importing prompt_build from here would
    # close a cycle (it imports this module for the two values), and this
    # check is about what the FILE says, before any substitution.
    try:
        core_src = (HERE / "process_core.md").read_text(encoding="utf-8")
    except OSError as e:                                       # noqa: BLE001
        bad.append(f"process_core.md is unreadable ({e})")
    else:
        bad.extend(length_rule_faults(core_src))

    # EVERY DECLARED KNOB IS WELL FORMED — stage 4 (R382). A provider declares
    # its own tuning, so nothing else in the battery reads these declarations;
    # a default outside its own choices would be invisible until someone tried
    # to reset one.
    try:
        import providers as _P
        for pname, cls in sorted(_P.REGISTRY.items()):
            prov = cls()
            seen_k = set()
            for knob in getattr(prov, "TUNING", ()):
                if knob.key in seen_k:
                    bad.append(f"{pname}: tuning {knob.key!r} declared twice")
                seen_k.add(knob.key)
                if not knob.values:
                    bad.append(f"{pname}.{knob.key}: declares no choices")
                elif knob.default not in knob.values:
                    bad.append(f"{pname}.{knob.key}: default {knob.default!r} "
                               f"is not one of its own choices "
                               f"{knob.values}")
                if knob.applies not in ("immediate", "next_circle"):
                    bad.append(f"{pname}.{knob.key}: applies "
                               f"{knob.applies!r}")
            # The knob namespace must not collide with a plain setting, or
            # `/settings-update <name>` would be ambiguous.
            clash = seen_k & set(BY_KEY)
            if clash:
                bad.append(f"{pname}: tuning name(s) {sorted(clash)} collide "
                           f"with a plain setting of the same name")
    except Exception as e:                                     # noqa: BLE001
        bad.append(f"providers did not import: {e}")

    if PROVIDERS_SUPPORTED != ("Anthropic",):
        bad.append("PROVIDERS_SUPPORTED has grown — that is a ruling about "
                   "this project's local-only rule, not a code change")

    for b in bad:
        print("  FAIL  " + b)
    if bad:
        print(f"\nSETTINGS: {len(bad)} failure(s)")
        return 1
    print(f"  OK    {len(SPEC)} setting(s) declared, every owner resolves, "
          f"every call site reads, no verb is proposable")
    return 0
...
# added to the dispatcher and forgotten here: test_settings.py asserts this
