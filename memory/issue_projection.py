#!/usr/bin/env python3
"""
issue_projection.py — verify the live issue graph is sound to project into a
prompt, and provide the projections `circle.py` builds Block 2 from.

    python memory/issue_projection.py    verify SHAPE and PROVENANCE
...
is gone, and with it the one thing this module used to WRITE. circle.py's
`build_briefing()` calls straight into `block()`, `project_working_set()`,
`issue_relationship_brief()` and `narrative()` at
prompt-assembly time — no projection step, no file in between to go stale or
...
WHAT `main()` ASSERTS, over the live graph directly
    ...
    SHAPE      every live node has a Label and an absence clause. The gate
               ...
               `block()` hand a part an empty field.
...
PREAMBLE = """\
*Projected from `issues/` by `memory/issue_projection.py`. **What is OWED.**
...
def _transcript(ref: str) -> pathlib.Path:
    ...
    Approved 2026-08-03, mirroring `issue_gate.source_of()` — and since
...
# live_edges() — were three more regexes over a format with no grammar.
live_nodes = S.live_nodes


def block(p: pathlib.Path) -> tuple[str, list[str]]:
    """(projected block, failures)."""
    d = S.load(p)
    fails = []
    lab = d.get("label", "")
    ruled = d.get("label_ruled", "")
    ab = S.unwrap(d.get("absence", ""))
    if not lab:
...
def normalise_id(x: str) -> str:
    """`2`, `nPPPP`, `L_n9999` -> the bare id. Prefix is presentation."""
    x = x.strip()
...
def parse_working_set_pairs(s: str) -> list[tuple[str, str]]:
    """[(as typed, normalised)] — the split done ONCE, both views returned.

    A REFUSAL MUST ECHO WHAT WAS TYPED, 2026-08-20. normalise_id() strips a
    ...
    return [(x, normalise_id(x))
            for x in re.split(r"[,\s]+", s or "") if x.strip()]


def parse_working_set(s: str) -> list[str]:
    return [nid for _, nid in parse_working_set_pairs(s)]


def live_edges(d: dict) -> list[tuple[str, str]]:
...
def full_block(p: pathlib.Path, d: "dict | None" = None) -> str:
    ...
    thinking. `d` reuses a doc graph() already parsed (tier 5 #58)."""
    d = S.load(p) if d is None else d
    lab, ruled = d.get("label", ""), d.get("label_ruled", "")
    prov = (f"*ruled by {ID.DISPLAY} in {ruled}*" if ruled
            else "*the 2026-07-27 derivation's wording — NOT YET RULED*")
    L = [f"### {p.stem} — {lab}", prov, "",
         S.unwrap(d["description"]), "",
         f"**Absent when:** {S.unwrap(d.get('absence',''))}"]
    e = live_edges(d)
    if e:
        L += ["", "**Live edges:** "
...
def brief_block(p: pathlib.Path, d: "dict | None" = None) -> str:
    """A periphery node — name and description only, by ruling.
    `d` reuses a doc graph() already parsed (tier 5 #58)."""
    d = S.load(p) if d is None else d
...
def project_working_set(chosen: list[str]) -> tuple[str, list[str]]:
    ...
    Blank chosen means the whole live graph, each node shown via `block()`
    ...
    The pre-commit hook runs issue_projection.py's own main() on every commit
    ...
    if not chosen:
        ...
        if fails:
            raise RuntimeError(
                ...
                "failure(s) issue_projection.py's own gate should have caught:\n"
                + "\n".join(f"  - {x}" for x in fails))
        return "\n".join(parts).rstrip("\n") + "\n", []
    # ONE graph build feeds resolve() and both block renderers (tier 5
    # #58): this used to parse every issues/*.toml twice at the top and
    # once more per rendered node.
    g = graph()
    focus, per, unknown = resolve(chosen, g)
    L = [SECTION, "", WS_PREAMBLE]
    L += [f"*Working set: {', '.join(focus)}"
          + (f" · periphery {', '.join(per)}" if per else " · no periphery — "
             "no live edge reaches these, which is itself worth a circle")
          + ".*", ""]
    for nid in focus:
        L += [full_block(g[nid]["path"], g[nid]["doc"]), ""]
    if per:
        L += ["**Related — name and description only:**", ""]
        for nid in per:
            L += [brief_block(g[nid]["path"], g[nid]["doc"]), ""]
    return "\n".join(L).rstrip("\n") + "\n", unknown
...
# from `live_edges()`. The glosses are the one hand-written thing here,
...
# issue_projection.md admitted as much — and gate_edge_types(), the
...
if set(GLOSS) != set(S.EDGE_TYPES) or {t for _, t in CHOOSE} != set(S.EDGE_TYPES):
    raise RuntimeError(
        "issue_projection's GLOSS/CHOOSE vocabulary no longer matches "
...
def issue_relationship_brief(chosen: "list[str] | None" = None) -> str:
    ...
    def _for_room(basis: str) -> str:
        ...
        return re.sub(r"\s*\((?:proposed by the derivation|the derivation "
                      r"recorded this)[^)]*\)", "", S.unwrap(basis)).strip()
    ...
    if opens:
        ...
        for what, q in opens:
            L.append(f"- {what} — {S.unwrap(q['question'])}")
            if q.get("type"):
