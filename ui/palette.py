...
`system_unique_home_verify.py` reads only Python, so a hand-copied hex there could drift
...
def ui_css_verify(path: pathlib.Path | None = None) -> list[str]:
